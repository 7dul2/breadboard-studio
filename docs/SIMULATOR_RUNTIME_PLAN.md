...
### 14.1 阶段 4 — 已完成（2026-09-09）

原文列的四项全部落地，逐项对应 `docs/STATUS.md` 的 S4.1–S4.6：

| 原计划项 | 实际交付 | 计划没预料到的事 |
| --- | --- | --- |
| 电阻作为二端导通元件 + 电流估算（连带 `output.led@1`） | S4.1 定义新增 `conduction`，连通性分成 `full`（电流能过去）与 `direct`（同一节点）两层 | `internal_nets` 是错的机制：用它会把电源经电阻接地判成死短路，同时让「两腿插进同一列」不再报错 |
| — | S4.2 顺带修好接地网络 | 接地网络在数字域里一直是 `Z`：网络只有被驱动才会重新求解，而接地网络从来没有驱动端，于是从项目第一天起就没被 settle 过 |
| 传感器行为模型与属性滑杆（SHT41/BMP390/LTR390） | S4.2/S4.3/S4.6 四个驱动（含 SEN66），滑杆范围进目录 `SimulationControlDef.range` | 三种 I²C 寻址形态（8 位命令字、寄存器指针、16 位命令字 + 每字 CRC）都得建模，只支持一种会让大半真实库跑不通 |
| 网络值时间线与断点 | S4.4 独立 `net-trace` 通道，三处有界且丢弃必计数 | 这条通道必须**排队不合并**——其它高频通道合并掉旧值只是少一帧，这里合并掉一条边就是时间线上的一个洞 |
| 输入事件录制回放 | S4.5 Worker 侧打时刻，回放在 `prepare` 与 `start` 之间装载 | 录制属于一次运行，开新会话必须清空，否则回放会把自己录进去 |

两条方法论上的教训写在这里，因为它们比任何一个器件都值钱：

1. **自证的测试。** BMP390 的器件端用带钳位的补偿函数二分产生原始值，测试又用同一个带钳位的函数解码——两个误差正好抵消。真实解码器（数据手册 §8.5、Adafruit）在 −40 °C 读到的是 −40.13 °C。凡是「同一个函数正反各走一遍」的往返测试，证明的只是自洽；解码必须用独立的形式。
2. **取证胜过回忆，对抗性审查胜过取证。** BMP390 的四个 agent 把数据手册与 Bosch 官方驱动下载到本地、我再自己读原文，避开了「注释写 1 / 2^8、代码却是乘 256」这类陷阱；随后四个对抗性审查 agent 又在成品代码里找出 **7 个真错误**，每条都带源码行号，两个 lens 独立收敛到同一组。

### 14.2 阶段 5 — 真实固件后端：已调查，建议不做（2026-09-09）

RFC 与技术验证已完成，见 **`docs/PHASE5_RFC.md`**。三条路线的判定：

- **ESP-IDF QEMU → WASM：否决。** 浏览器里跑 QEMU 本身可行且已被现场验证，但 ESP32-S3 那一半死了——`hw/gpio/esp32_gpio.c` 全文 109 行、写处理函数字面上是空的，`hw/xtensa/esp32s3.c` 实例化了 26 个外设而其中没有 I²C。本项目整个建立其上的两个外设，恰好是这条路线上不存在的两个，而且是政策不是积压（Espressif issue #37 自 2022 开着）。另有 GPL-2.0 与本仓库 MIT 的许可证冲突。
- **指令集模拟器：唯一能满足接口的**，不引入第二个 wasm，宿主那一半完全不用改；但地基是一个 2.5 周新、1 star、其文档自陈「没有 I²C 模型」的仓库。值得两周一次性验证，不值得当项目做。
- **WebSerial：重新归类。** `SimulationBackend` 十个方法里有五个它给不了（没有时钟可驱动，也无法在网络跳变处停住真实 MCU），不应注册成后端；但作为「烧录 + 看串口 + 硬复位」的独立功能一周可交付。

**结论：阶段 5 标记为「已调查并谢绝」。** 计划要求的是先验证再决定，验证的结论可以是不做——这是合法结局，不是失败。真正的障碍是结构性的：浏览器里没有任何东西能编译 Xtensa，所以「真实固件」只能是「别人编译好的 `.bin`」；而有 `.bin` 的人有工具链，有工具链的人有板子和 `idf.py monitor`。

**RFC 的建议里唯一动手做的那条（2026-09-09）**：WebSerial 改名为独立模式「实机」，落在 `apps/web/src/hardware/`——`serial-link.ts`（支持检测、`SerialLink` 接口、流式 UTF-8 行解码与 2000 行环形缓冲）、`browser-serial.ts`（唯一需要浏览器与真板的一层）、`hardwareStore.ts`、`HardwarePanel.tsx`。三条自我约束写死在代码与测试里：**不实现也不注册 `SimulationBackend`**（十个方法里有五个它给不了）；**不喂器件模型**（真板的引脚接的是桌上的真元件，画布上的 OLED 与传感器不在那个电路里）；**离开「实机」就挂断**（看不见的串口是惊吓，重连只要一次点击）。做的是卡里的「看串口 + 硬复位」，**烧录没做**，所以 esptool-js 没有引入，入口 chunk 703,674 → 746,493 B。e2e 用 `addInitScript` 注入一个假的 `navigator.serial`，被测的是真的适配器与真的行解码器，而不是它们的替身；复位那条断言钉的是 RTS 拉低、DTR 保持不动的时序——反过来就是进下载模式而不是正常启动。

判断路线时用的硬约束（留档，将来重启此题仍适用）：只有静态托管（GitHub Pages，无服务端）；`SimulationBackend` 的 `pause`/`step`/`reset`/`setSpeed` 都作用在整数微秒虚拟时钟上；确定性是有测试钉着的承诺，录制回放依赖它；入口 chunk 卡在 900,000 B，构建里只允许一个 wasm。任何一条守不住的路线，必须把守不住的部分写在最前面。
