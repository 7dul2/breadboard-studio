...
- 两条线在图上交叉不导通；一个孔只能插一根线（`wire_hole_conflict`）。**例外**：洞洞板上已被引脚占用的焊盘允许第二根导线落地（焊接面可再焊一根线），此时不报 `wire_hole_conflict`；但若该焊盘上已有两根导线，则第三根报 `wire_hole_conflict`。
...
- **渲染场景选项**：`SceneOptions` 含 `dimUnhighlighted: boolean`，为 `true` 时未被选中/未被高亮的元件与导线降低不透明度，突出当前选中对象；洞洞板上被导线覆盖的焊盘也按此规则淡化。
