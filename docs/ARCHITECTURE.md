...
- 可拆拼装式面包板不使用“隐藏整板的一部分”表达。中间接线板和 `+/−` 电源条各自是独立 `BoardDefinition` / `BoardInstance`，所以可分别移动、旋转、复制和删除；贴合只改变几何位置，不会添加跨板导通边。两块中间板之间夹一条 12.70 mm 电源条时，上板 `h` 与下板 `b` 的行中心距为 25.40 mm。
...
- **洞洞板例外**：洞洞板（`render.style === "perfboard"`）的每个焊盘是独立导通组（一个孔 = 一个组），所以上述规则中的「同列五孔」不适用——洞洞板上不存在隐式导通。此外，导线端点落在**已被引脚占用**的洞洞板焊盘上是合法的（不报 `wire_endpoint_occupied`），因为焊接面本来就可以在同一焊点再焊一根线；但同一焊盘上的**第二根导线**仍报 `wire_hole_conflict`。
...
- 两条线在图上交叉不导通；一个孔只能插一根线（`wire_hole_conflict`）。**例外**：洞洞板上已被引脚占用的焊盘允许第一根导线落地（焊接面可在同一焊点再焊一根线），此时不报 `wire_hole_conflict`；该焊盘上再插第二根导线就报 `wire_hole_conflict`。
...
- **渲染场景选项**：`SceneOptions` 含可选的 `dimUnhighlighted: boolean`，为 `true` 时未被选中/未被高亮的**导线**（含线号标注）降低不透明度到 0.16，突出当前选中对象；它只作用于导线路径，不压暗元件与焊盘。
