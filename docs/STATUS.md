...
- 仿真只做到阶段 1：程序真的会跑，但只有 ESP32-S3 的 GPIO 与板载 RGB 有行为。I²C 一律返回总线不可用（`i2c_bus_unavailable`），TTP223 与 OLED 还没有驱动（各报一次 `unsupported_device`），所以夹具的屏幕仍是黑的。按钮与触摸是 M-S2，I²C 与 OLED 是 M-S3。
