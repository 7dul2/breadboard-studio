...
最后更新：2026-09-09。环境：macOS 26.6 (arm64)、Node 26.0.0、pnpm 11.25.0、Chromium 153（Playwright 1.63）。
...
| `pnpm build` + `pnpm check:dist` | 在 `/breadboard-studio/` 子路径下加载示例、24 根线、5 个网络、0 控制台错误；并真的启动一次仿真（`nowUs` 推进到 100,000）走通「子路径 + 生产构建 + Worker + wasm」。主 chunk 726,786 B（阈值 900,000）、不含 quickjs/sucrase 字样、`.wasm` 恰好 1 个 |
